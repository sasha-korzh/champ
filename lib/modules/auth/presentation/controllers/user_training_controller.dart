...
class UserTrainingController extends GetxController {
 ...
 final User _user;
  ...
  UserTrainingController(this._user, this._trainingUsecase);
  ...
  @override
  void onInit() {
    print('UserTrainingController onInit');
    initFeed();
    super.onInit();
  }
  ...
  Future<void> fetchTrainingPage() async {
    if (!lastPage.value) {
      final result = await _trainingUsecase.getCreatedTrainingByUserId(_user.id, currentLastEvaluatedKey.value);
      print('UserTrainingController: fetch post page');
      result.fold(
        (failure) { 
          print('UserTrainingController: result: Failure');
          showErrorSnackbar(failure);
        },
        (trainingPage) {
          print('UserTrainingController: result: Success');

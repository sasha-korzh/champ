...
class TrainingFeedPage extends StatelessWidget {
  final TrainingFeedController feedController = Get.find();
          ...
          child: SmartRefresher(
            ...
            child: ListView.builder(
              itemBuilder: (context, index) {
                 final training = feedController.trainings[index];
                 return GestureDetector(
                   onTap: () {
                     Get.toNamed(Routes.trainingPage, arguments: training);
                   },
                   child: Center(
                     child: Padding(
                       padding: EdgeInsets.only(bottom: su.setHeight(44)),
                       child: TrainingListItem(training: training),
                     ),
                   ),
                 );
              },
              itemCount: feedController.trainings.length,
            ),
          ),
        ),
      )),
